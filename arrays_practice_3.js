...
// console.log(
//     users.find( ({ firstName, lastName }) => firstName==="Madison" && lastName === "Marshall").id
// )

// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)

// console.log(
//     users.find( ( {id} ) => id === comments[0].userId )
// )
...
// console.log(
//     users.find( ( { id } ) => id === comments
//     .find( ( {text} ) => text === "OK great thanks").userId)
// )

// 4) Add the user's first and last name to each comment in the comments array

// console.log(
//     comments.map( comment => ({...comment, firstName: users.find(
//         ( { id } ) => id === comment.userId ).firstName, lastName: users.find(
//             ( { id } ) => id === comment.userId ).lastName})
//     )
// )

// const answer = comments.map( comment => {
//     const {firstName, lastName} = users.find( ({id}) => id === comment.userId ) 
...
//     const found = comments.some( ({userId}) => userId === user.id);
